...
| **URL** | `wss://lanser.fun/block/ws/gateway` |
| **浏览器API** | `WebSocket` (无需额外库) |
| **消息格式** | JSON |
...
| **协议** | 原生 WebSocket (RFC 6455) |
| **URL** | `wss://lanser.fun/block/ws/gateway` |
| **Python库** | `websocket-client` |
| **认证** | HTTP Header: `X-Vehicle-ID` |
| **消息格式** | JSON |
...
|------|---------------|-----------|
| **握手** | HTTP Upgrade (一次) | HTTP GET + SID (多次) |
| **协议** | 标准RFC 6455 | 自定义协议 |
| **消息格式** | 任意文本/二进制 | Socket.IO数据包 |
| **心跳** | 应用层实现 | 内置ping/pong |
| **房间** | 应用层实现 | 内置rooms |
| **重连** | 需手动实现 | 内置重连机制 |
| **跨浏览器** | 完全兼容 | 需要客户端库 |
...
*创建日期: 2026-02-19*
*更新日期: 2026-02-21*
*更新内容: 明确前端和后端均使用原生WebSocket协议，添加协议技术栈说明*
